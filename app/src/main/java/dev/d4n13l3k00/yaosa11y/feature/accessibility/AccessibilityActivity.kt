...
import dev.d4n13l3k00.yaosa11y.core.ui.redirectDpadRightTo
...
class AccessibilityActivity : Activity() {
        ...
        override fun onChange(selfChange: Boolean) {
            ...
            if (::rootHookManager.isInitialized && rootHookManager.shouldBeEnabled()) {
                rootHookManager.reconcileAsync { result ->
                    if (result.changed) postIfAlive { renderServices() }
                }
            }
        }
    }
    ...
    private fun renderServices() {
        ...
        val previousFocus = serviceRows.entries.firstOrNull { (_, views) ->
            views.row === focusedView || views.protectionButton === focusedView
        }
        val previousComponent = previousFocus?.key
        val protectionHadFocus = previousFocus?.value?.protectionButton === focusedView
        ...
        serviceRows.clear()
        entries.forEach {
            val views = serviceRow(it, canWrite, it.component in protected)
            serviceRows[it.component] = views
            serviceList.addView(views.row)
        }
        wireServiceFocus(serviceRows.values.toList())

        val focusTarget = previousComponent?.let(serviceRows::get)?.let { views ->
            if (protectionHadFocus) views.protectionButton else views.row
        } ?: if (pendingInitialServiceFocus) serviceRows.values.firstOrNull()?.row else null
        pendingInitialServiceFocus = false
    ...
    private fun serviceRow(
        ...
        val componentView = TextView(this).apply {
            text = buildString {
        ...
        row.redirectDpadRightTo(protectionButton)
        protectionButton.redirectDpadLeftTo(row)
        row.nextFocusLeftId = row.id
        protectionButton.nextFocusRightId = protectionButton.id
        row.setOnFocusChangeListener { _, focused ->
            titleView.isSelected = focused
            componentView.isSelected = focused
        }
        protectionButton.setOnFocusChangeListener { _, focused ->
            titleView.isSelected = focused
            componentView.isSelected = focused
        }
        ...
        return ServiceRowViews(
            row = row,
            protectionButton = protectionButton,
        )
    }

    private fun wireServiceFocus(rows: List<ServiceRowViews>) {
